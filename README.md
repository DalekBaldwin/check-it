...
Generators are constructed using the `generator` macro, which takes a generator type spec as a single argument. This is not exactly the same as a Common Lisp type spec, but a lot of the ones you write will be valid Common Lisp type specs. Values are generated by calling `generate` on generator objects.
